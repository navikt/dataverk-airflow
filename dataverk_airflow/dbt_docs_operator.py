...
def dbt_docs_operator(
        dag: DAG,
        name: str,
        dbt: dict = {"profiles_dir": ".", "project_dir": ".", "team": os.getenv("TEAM"), "env": "dev"},
        repo: str = None,
        image: str = None,
        branch: str = "main",
        email: str = None,
    ...
    """Operator for running dbt docs generate and publish to dbt.intern(.dev).nav.no

    Depending on whether you define dev or prod as the env in the dbt input dictionary either
    dbt.intern.dev.nav.no or dbt.intern.nav.no will be added to the list of allowed hosts for the
    Airflow worker.
    ...
    if env == "dev":
        ...
        raise KeyError(f"env parameter is required for the publish docs command, must be set to either dev or prod")
    
    allowlist.append(host)
    
    cmds = [
        f"dbt docs generate --profiles-dir {profiles_dir} --project-dir {project_dir}",
